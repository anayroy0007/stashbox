...
type Site struct {
	HtmlBody []byte //warning: exported type Site should have comment or be unexported (golint)
	TextBody []byte 
	Url      string
	Title    string
}
...
type Crawler struct {
	Urls    []string //warning: exported type Crawler should have comment or be unexported (golint)
	Archive string
	Sites   []Site
}

func NewCrawler(archive string) (Crawler, error) {
	return Crawler{  //warning: exported function NewCrawler should have comment or be unexported (golint)
		Archive: archive,
	}, nil
}

func (c *Crawler) Save() error {
	ensureArchive(c.Archive)   //warning: exported method Crawler.Save should have comment or be unexported (golint)
...
func (c *Crawler) Crawl() error {
	for _, u := range c.Urls {
		...
		var site Site  //warning: exported method Crawler.AddUrl should have comment or be unexported (golint)

		htmlBody, err := getHtmlBody(u)
		if err != nil {
			return err //warning: exported method Crawler.Crawl should have comment or be unexported (golint)
		}
